...
// https://github.com/LordNoteworthy/al-khaser/blob/master/al-khaser/AntiDebug/TLS_callbacks.cpp
...
bool tls_callback_thread()
{
    ...
    int fuse = 0;
    while (tls_callback_thread_event == NULL && ++fuse != BLOWN)
    {
        SwitchToThread();
    }
    if (fuse >= BLOWN)
    ...
    */
    ...
    return tls_callback_thread_data != 0xDEADBEEF;
}

bool tls_callback_process()
{
    const int BLOWN = 1000;

    int fuse = 0;
    while (tls_callback_process_event == NULL && ++fuse != BLOWN)
    {
        SwitchToThread();
    }
    if (fuse >= BLOWN)
    ...
    */
    ...
    return tls_callback_process_data != 0xDEADBEEF;
}
